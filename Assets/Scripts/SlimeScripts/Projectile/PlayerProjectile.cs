    ...
    private void OnTriggerEnter2D(Collider2D other)
    {
        if (whatIsCrashable.CompareGameObjectLayer(other.gameObject))
        {
            if (whatIsEnemy.CompareGameObjectLayer(other.gameObject))
            {
                ...
                if (enemy != null)
                {
                    SlimeGameManager.Instance.Player.GiveDamage(enemy, moveVec, transform.position, SlimeGameManager.Instance.Player.PlayerStat.MinDamage, SlimeGameManager.Instance.Player.PlayerStat.MaxDamage, 0f, knockBackPower, true);
